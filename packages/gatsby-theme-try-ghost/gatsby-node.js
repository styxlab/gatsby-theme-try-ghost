...
const createPostPages = (createPage, posts, basePath, template, tags, collectionPath = ``) => {
    ...
    const path = resolveUrl(basePath) + resolveUrl(collectionPath)

    posts.forEach(({ node }, i) => {
        // Determine the routing structure from
        // Ghost CMS by analyzing the url field
        const url = resolveUrl(path, node.slug, node.url)
...
//ToDo: count duplicates in postTags, postAuthors and set count as postCount
//ToDo: resolveUrl must include collectionPath
const createCollection = (createPage, basePath, collectionPath, data, templates, allTags, postsPerPage) => {
...
exports.createPages = async ({ graphql, actions }, themeOptions) => {
    ...
    collections.forEach((collection) => {
        ...
        createCollection(createPage, basePath, collection.path, collectionData.primary, templates, data.tags, postsPerPage)
